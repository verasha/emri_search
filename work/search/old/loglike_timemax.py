...
class LogLikeTimeMax:
    ...
    This version uses time-maximized inner products (FFT correlation) for all statistics.
    Useful for detection or when extrinsic parameters (time/phase) are uncertain.
    """
    ...
    def __call__(self, theta_template):
        ...
        Uses time-maximized versions of all inner products (Xstat_timemax, Xmstat_timemax, etc.)

        Parameters:
        theta_template: Template parameters (m1, m2, a, p0, e0, xI0, dist, qS, phiS, qK, phiK, Phi_phi0, Phi_theta0, Phi_r0)

        Returns:
        float: Time-maximized log-likelihood value
        """
        ...
        # TIME-MAXIMIZED: Use rhostat_timemax instead of rhostat
        ...
        # TIME-MAXIMIZED: Calculate rho_m with time maximization
        # Note: We could use rhostat_timemax for each mode, but for consistency with F-stat
        # we use regular rhostat (since modes are already summed over m)
        ...
        if self.verbose:
            ...
            for i, wf in enumerate(waveform_per_mode):
                ...
                # Calculate time-maximized inner product manually for debugging
                inner_prod = self.gwf.inner_timemax(wf, wf)
                print(f"  Mode {i}: max(|h|) = {wf_max}, <hf|hf>_timemax = {inner_prod}, rho = {rho_m[i]}")
        ...
        # TIME-MAXIMIZED: Calculate Xm with time maximization
        X_modes = self.gwf.Xmstat_timemax(self.signal, waveform_per_mode, rho_m)

        # TIME-MAXIMIZED: Calculate X statistic with time maximization
        ...
        if self.verbose:
            ...
            print(f"X_scalar (time-maximized): {X_scalar}")
        ...
        if self.verbose:
            print(f"Time-maximized Log-likelihood: {logl_res}")
