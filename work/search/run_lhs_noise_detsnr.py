"""
LHS evaluation script: computes time-maximized X statistic for each template.

X_timemax(theta) = max_{tau,phi} |<d|h(theta;tau,phi)>| / sqrt(<h|h>)
                 = Xstat_timemax(d, h(theta))

No chi_sq suppression, no mode selection.
Faster per evaluation than the full loglike since only one waveform call is needed.

Usage:
    python run_lhs_noise_detsnr.py                  # fresh run
    python run_lhs_noise_detsnr.py --resume         # resume from latest checkpoint
...
parser.add_argument("--outdir",     type=str,
                    default="/scratch/e1498138/lhs/noise/overlap/ckpt_1e5",
...
# ---------------------------------------------------------------------------
# X_timemax: max_{tau,phi} |<d|h(tau,phi)>| / sqrt(<h|h>)
# ---------------------------------------------------------------------------
def compute_det_snr(params):
    """
    params: (n, 5) array of [log10_m1, log10_m2, a, p0, e0]
    returns: (n,) array of X_timemax values (nan on failure)
    """
    ...
    for i in range(n):
        try:
            ...
            x_stat = gwf.Xstat_timemax(signal, h_temp)
