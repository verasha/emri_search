...
loglike_obj = loglike_pure.LogLikeTimeMax(
    params_star,
    waveform_gen_comb,
    gwf,
    verbose=False,
...
S = 1.0   # annealing factor if needed
_TARGET_LOGLIKE = data_snr * S   # stop once annealed loglike hits target
_EARLY_STOP_HIT = False

# Load paris2 stage
# paris2(and 2_1): Up to S=100 stages
# anneal12: up to S=30
...
# Max ld point
all_pts_u  = sampler_p2_1.searched_points_list[0]
all_logden = sampler_p2_1.searched_log_densities_list[0]
maxld_idx  = np.argmax(all_logden)
mu_center  = prior_transform(all_pts_u[maxld_idx].reshape(1, -1))[0]
print(f'paris2 stage maxld internal logden: {all_logden[maxld_idx]:.4f}')
print(f'paris2 maxld point (phys):    {mu_center}')
...
cov_posterior = np.cov(samples_p2_1[idx_rs].T)  # physical-space posterior covariance
print('paris2 posterior 1-sigma (diag):', np.sqrt(np.diag(cov_posterior)))

del sampler_p2_1, samples_p2_1, weights_p2_1, idx_rs  # free memory
...
# ─────────────────────────────────────────────
# log_density: NON-phasemax, S=1, + ellipse guard
# ─────────────────────────────────────────────
...
# Start from paris2_1 maxld point
...
sampler.run_sampling(
    ...
    savepath='./intrinsic_ffunc_3mth_snr32_paris3_3',
    print_iter=100,
    callback=paris3_callback,
    external_lhs_points=external_lhs_points,
    external_lhs_log_densities=external_lhs_log_densities,
    stop_max_ld_stable_iters=int(2e4)
)
print('Done.')
