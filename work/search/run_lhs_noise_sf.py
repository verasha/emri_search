...
parser.add_argument("--n-segs",     type=int, default=12,       help="Number of time segments for S_N / Sf")
parser.add_argument("--outdir",     type=str,
                    default="/scratch/e1498138/lhs/noise/sf/ckpt_1e5",
...
# LogLike instance: runs mode selection once (at true params) and provides
# _generate_selected_waveforms for the per-mode templates that Sf needs.
print('Building LogLike (mode selection)...')
loglike_obj = LogLike(params_star, waveform_response, gwf,
                      add_noise=False, verbose=True)
selected = loglike_obj.mode_select if loglike_obj.mode_select else loglike_obj.selected_labels
print(f"Selected mode groups: {selected}")
...
theta_true = list(params_star)
hm_true = loglike_obj._generate_selected_waveforms(theta_true, selected)
sf_true = float(gwf.Sf(signal, h_true, hm_true, N_seg=N_segs, common_tau=True))
print(f"Sf (N_seg={N_segs}, common_tau) at true params: {sf_true:.2f}")
del h_true, hf_true, hm_true
...
def compute_det_snr(params):
    ...
    returns: (n,) array of Sf values (nan on failure)

    Semi-coherent f-statistic with common-tau: the per-segment lags are fixed
    by the full template and every mode is evaluated at those same lags, so the
    chi-square term collapses at the truth instead of being flattened by
    independent per-mode noise maximization.
    """
    ...
    for i in range(n):
        try:
            ...
            out[i] = float(gwf.Sf(signal, h_temp, hm_arr,
