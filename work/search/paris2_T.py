...
import GWfuncs
import loglike_pure_T  # NONTIME-MAXIMIZED VERSION, with T as param
...
loglike_obj = loglike_pure_T.LogLike(
    params_star,
    waveform_gen_comb,
    gwf,
    verbose=False,
...
ndim = 5
n_seed = 1
paris1_cov = np.array([[ 2.85872992e-01, -1.04243882e-02, -2.22289961e-03,
          3.54865212e-03, -2.90216333e-03],
        [-1.04243882e-02,  3.18079961e-01, -1.45275450e-04,
          2.07334347e-03,  4.19664680e-03],
        [-2.22289961e-03, -1.45275450e-04,  2.80277794e-01,
         -6.40906635e-03,  1.02258403e-03],
        [ 3.54865212e-03,  2.07334347e-03, -6.40906635e-03,
          2.98482301e-01,  6.72146430e-03],
        [-2.90216333e-03,  4.19664680e-03,  1.02258403e-03,
          6.72146430e-03,  3.22793858e-01]])
init_cov_list = [paris1_cov.copy()]
...
# Start from best fit
best_fit = [6.02970234, 1.27508141, 0.88321051, 9.49373423, 0.24784163]
...
def anneal_callback(sampler, i):
    ...
    if i - state['ref_iter'] >= stuck_iters:
        if stage < len(T_schedule) - 1:
            ...
            # Re-evaluate best live point at new T so max_ld reflects new landscape
            for j in range(sampler.n_proc):
                n = sampler.element_num_list[j]
                if n == 0:
                    continue
                lds = sampler.searched_log_densities_list[j][:n]
                best_idx = int(np.argmax(lds))
                best_pt  = sampler.searched_points_list[j][best_idx:best_idx + 1]
                new_ld   = sampler.log_density_func(best_pt)[0]
                print(f"  proc {j}: best point re-eval at T={new_T:.2f}yr -> {new_ld:.5f} (was {lds[best_idx]:.5f})", flush=True)
                if np.isfinite(new_ld):
                    sampler.searched_log_densities_list[j][best_idx] = new_ld
                    sampler.max_logden_list[j] = new_ld
            sampler.loglike_normalization = max(sampler.max_logden_list)

            state['ref_max_ld'] = sampler.max_logden_list[0]
            print(f"Stuck {stuck_iters} iters at T={T_cur:.2f}yr. Jumping -> T={new_T:.2f}yr at iter {i}, "
                  f"new max_ld={sampler.max_logden_list[0]:.5f}", flush=True)
        else:
...
savepath = dir_scratch+'paris2_T/int_3mth_1'
