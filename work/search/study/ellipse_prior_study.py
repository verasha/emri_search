...
Compares:
  ...
  2. Ellipsoidal prior derived from weighted covariance of paris2 samples

Sections:
  A. Full setup (loglike_pure / GWfuncs_pure, non-phasemax)
  B. Load paris2 samples + compute weighted covariance
  C. Ellipse construction helpers
  D. Corner plot: ellipse samples vs paris2 posterior
...
# ─────────────────────────────────────────────
# A. Full setup (loglike_timemax / GWfuncs — matches paris2)
# ─────────────────────────────────────────────
...
# ─────────────────────────────────────────────
# B. Load paris2 samples & compute covariance
# ─────────────────────────────────────────────
...
print(f'paris2 maxld = {np.max(_logdens):.4f}')
...
cov_paris2 = np.cov(posterior_samples.T)   # keep annealed (tighter)

print("mu_center (maxld):         ", mu_center)
print("Posterior 1-sigma (diag):  ", np.sqrt(np.diag(cov_paris2)))
...
cov_plot     = cov_paris2 + np.diag((smooth_sigma * bin_widths) ** 2)

print("KDE bin widths:            ", bin_widths)
print("True 1-sigma:              ", np.sqrt(np.diag(cov_paris2)))
...
# ─────────────────────────────────────────────
# D. Corner plot: 2D marginal ellipse patches vs paris2 posterior
# ─────────────────────────────────────────────
...
green_patch = mpatches.Patch(color='green', label='paris2 posterior')
...
blue_line   = plt.Line2D([0], [0], color='blue',  ls='--', label='paris2 maxld point')
...
fig.suptitle('paris2 posterior vs Ellipsoidal prior (2D marginals)', fontsize=12)
plt.savefig('./search/plots/ellipse_prior_corner.png', dpi=150, bbox_inches='tight')
print("Saved: ./search/plots/ellipse_prior_corner.png")
...
plt.tight_layout()
plt.savefig('./search/plots/ellipse_volume_efficiency.png', dpi=150, bbox_inches='tight')
print("Saved: ./search/plots/ellipse_volume_efficiency.png")

print("\n=== Volume summary ===")
for ns in [1.0, 3.0, 5.0]:
    V_ell   = ellipse_volume(cov_paris2, n_sigma=ns)
    V_box_c = box_volume_from_cov(cov_paris2, n_sigma=ns)
