...
if args.resume:
    ckpts = sorted(glob.glob(os.path.join(args.outdir, "lhs_noise_paris1_ckpt_*.pkl")))
    if ckpts:
        latest = ckpts[-1]
...
for i in range(start_idx, n_total, batch_size):
    ...
    if batch_count % save_every == 0 or end == n_total:
        ckpt_path = os.path.join(args.outdir, f"lhs_noise_paris1_ckpt_{end:06d}.pkl")
...
# ---------------------------------------------------------------------------
out_path = os.path.join(args.outdir, "lhs_noise_paris1_final.pkl")
