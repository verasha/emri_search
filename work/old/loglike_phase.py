...
class LogLikePhase:
    """
    Phase-maximized log-likelihood. Uses |<x|h>| instead of Re(<x|h>) for X_scalar.
    All other statistics identical to LogLikePure.
    """
    ...
    def __call__(self, theta_template):
        """
        Phase-maximized log-likelihood: uses |<x|h>| for X_scalar.
        """
        if self.mode_select:
        ...
        # PHASE-MAXIMIZED: mirrors loglike_timemax but abs() instead of inner_timemax
        X_scalar = self.gwf.Xstat(self.signal, h_temp)
        X_modes = self.gwf.Xmstat_phasemax(self.signal, waveform_per_mode, rho_m)
        ...
        if self.verbose:
            ...
            print(f"X_scalar (phase-max): {X_scalar}")
        ...
        if self.verbose:
            print(f"Log-likelihood (phase-max): {logl_res}")
