...
class LogLike:
    """
    Pure (no time-maximization) log-likelihood with T as a free parameter.

    Signal and gwf are initialized at T_max (gwf.T).
    ...
    def __call__(self, theta_template):
        """
        Evaluate pure (no time-maximization) log-likelihood.
        ...
        *phys_params, T_prop = theta_template
        ...
        waveform_per_mode = [
            self._pad_to_N(waveform_combined[:, i])
        ...
        X_scalar = self.gwf.inner(xf, hf, return_complex=False) / self.gwf.xp.sqrt(self.gwf.inner(hf, hf))
        ...
        if self.verbose:
            print(f"Log-likelihood (pure, T={T_prop:.4g}yr): {logl_res:.6g}")
